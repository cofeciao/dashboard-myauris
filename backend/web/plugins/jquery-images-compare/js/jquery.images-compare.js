    ...
    function ImagesCompare(element, options) {
        ...
        function patchSize() {
            var imgRef = backElement.find('img').first(),
                scrWid = $(window).width(),
                scrHei = $(window).height(),
                titHei = $('.js-img-compare-title').height();
            var newWid, newHei;

            if (scrWid > 768) {
                newHei = scrHei - titHei - 121;
                newWid = newHei * 3 / 4;
            } else {
                newWid = imgRef[0].clientWidth;
                newHei = newWid * 3 / 2;
            }

            setSize(imgRef.width(), imgRef.height(), imgRef.naturalWidth(), imgRef.naturalHeight());
            element.css('max-width', newWid + 'px');
            element.css('max-height', newHei + 'px');
            frontElement.width(newWid);
            frontElement.height(newHei);
            frontElement.next('.images-compare-after').width(newWid);
            frontElement.next('.images-compare-after').height(newHei);
        }
