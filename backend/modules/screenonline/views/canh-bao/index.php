        ...
        <div class="card-content h-100">
            <div class="row m-0 h-100">
                <div class="col-left col-sm-6 col-12 d-flex flex-wrap h-100">
                    <div class="row top m-0 w-100 h-50 p-0">
                        <div class="row1 w-100 d-flex clearfix">
                            <div class="col-sm-6 col-12 p-0">
                                <div class="card-box d-flex flex-wrap h-100 border">
                                    <h4 class="header-title w-100 h-20 text-center mb-0">Thời gian đặt hẹn đến quyết
                                        định
                                        làm</h4>
                        ...
                        <div class="row2 w-100 d-flex clearfix">
                            <div class="col-sm-6 col-12 p-0">
                                <div class="card-box d-flex flex-wrap h-100 border">
                                    <h4 class="header-title w-100 h-20 text-center mb-0">Trung bình thời gian làm thực
                                        tế
                            ...
                            <div class="row2 w-100 h-85 d-flex clearfix">
                                <div class="col-sm-6 col-12 p-0">
                                    <div class="card-box border text-white w-100 h-100 px-1">
                                        <div class="text-center w-100" style="font-size:24px;margin:10px 0 20px">Sales
                                        </div>
                ...
                <div class="col-right col-sm-6 col-12 h-100">
                    <div class="w-100 p-3 text-center text-white" style="font-size:36px;">Hệ thống kiểm soát tự động
                    </div>
                    <div class="card-box w-100 py-0 text-light warning-cols">
                        <div id="timeline" class="timeline-wrapper overflow-auto">
                            <ul class="list list-timeline h-100" id="warning-load">
                                <?= $this->render('warning', ['data' => $data]); ?>
                            </ul>
                        </div>
                    </div>
                </div>
...
<?php
$url_load = \yii\helpers\Url::toRoute('canh-bao/get-warning');
$script = <<< JS
    resize();
    ...
    if(Math.ceil(div.scrollTop + div.clientHeight) >= div.scrollHeight) {
        page ++;
        $('.warning-cols').myLoading({opacity:true});
        $.ajax({
            type: 'GET',
            url: '$url_load',
            data: {page:page}
            }).done(function(res) {
                if([null, undefined, ''].includes(res.trim())){
                    $('#timeline').unbind('scroll');
                } else {
                    $('#warning-load').append(res);
                }
               $('.warning-cols').myUnloading();
            });
    }
});
    function resize() {
        $('#timeline').height(window.innerHeight - 120);
    }
JS;
