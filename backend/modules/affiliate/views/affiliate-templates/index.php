...
/* @var $searchModel backend\modules\affiliate\models\search\AffiliateTemplatesSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Mẫu trang landing');
$this->params['breadcrumbs'][] = ['label' => Yii::t('backend', 'Affiliate'), 'url' => ['']];
...
<section id="dom">
    ...
    [
                        ...
                        <div style="margin-top:5px;border:1px solid #ccc;border-radius:3px">
                            <?php Pjax::begin(['id' => 'AffiliateTemplates', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
                            <?= MyGridView::widget([
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            return Html::a($model->name, ['view', 'id' => $model->id], ['data-pjax' => 0]) . ' ' .
                                                Html::a(
                                                    '<i class="ft-external-link">',
                                                    \yii\helpers\Url::to('https://donucuoi.vn/theme/' . $model->slug . '.html', true),
                                                    ['data-pjax' => 0, 'class' => 'pull-right mr-2', 'target' => '_blank', 'title' => 'Xem trang mẫu']
                                                );
                                        }
                                    ],
                                    'mota:html',
...
$script = <<< JS
var affiliate = new myGridView({
    pjaxId: '#AffiliateTemplates',
    urlChangePageSize: '$urlChangePageSize'
});
$('body').on('beforeSubmit', 'form#form-affiliate-templates', function(e){
    e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-affiliate-templates').serialize();
    
    $('#form-affiliate-templates').myLoading({opacity: true});
       
    $.ajax({
        url: $('#form-affiliate-templates').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-affiliate-templates').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: affiliate.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: affiliate.options.pjaxId});
                    }
