...
$cls_search_date_cre = $cls_search_date_payment = $cls_search_date_apm = 'hidden';
            ...
            <div class="form-search-col <?= $cls_col_1 ?>">
                ...
                <div class="form-group form-content row ml-0 mr-0">
                    <div class="f-title control-label">
                        <?= $form->field($model, 'type_search_payment')->radioList([
                            ...
                            'item' => function ($index, $label, $name, $checked, $value) {
                                return '
                                        <div class="custom-control custom-radio">
                                            <input type="radio" class="custom-control-input" id="' . $name . $index . '" name="' . $name . '" value="' . $value . '" data-name="type-search-payment" data-attr="date" data-type="payment" ' . ($checked == 1 ? 'checked' : '') . '>
                                            <label class="custom-control-label" for="' . $name . $index . '">' . ucfirst($label) . '</label>
                                        </div>
                                    ';
                            },
                    ...
                    <div class="f-content">
                        ...
                        <div class="date-range <?= $cls_search_date_payment ?> search-date-text text-to">-</div>
                        <div class="search-date payment_time_to date-range <?= $cls_search_date_payment ?>">
                            ...
                            <div class="f-content">
                                <div class="search-code">
                                    <?= $form->field($model, 'tam_ung', ['template' => '<div class="input-group">{input}<span class="input-group-addon clear-option"><span class="fa fa-times"></span></span></div>'])
                                        ->dropDownList(\yii\helpers\ArrayHelper::toArray(\backend\models\doanhthu\ThanhToanModel::THANHTOAN_TYPE), ['class' => 'form-control ui dropdown', 'prompt' => 'Hình thức'])->label(false) ?>
                                </div>
                            </div>
                        </div>
                    </div>
