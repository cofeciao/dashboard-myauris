...
<div class="form-search clinic-order">
    <div class="row">
        <div class="form-search-col <?= $cls_col_1 ?>">
            ...
            <div class="form-group form-content row ml-0 mr-0">
                <div class="f-title control-label">
                    <?= $form->field($model, 'type_search_payment')->radioList([
                        ...
                        'item' => function ($index, $label, $name, $checked, $value) {
                            return '
                                        <div class="custom-control custom-radio">
                                            <input type="radio" class="custom-control-input" id="' . $name . $index . '" name="' . $name . '" value="' . $value . '" data-name="type-search-date" data-attr="date" data-type="payment" ' . ($checked == 1 ? 'checked' : '') . '>
                                            <label class="custom-control-label" for="' . $name . $index . '">' . ucfirst($label) . '</label>
                                        </div>
                                    ';
                        },
                ...
                <div class="f-content">
                    <div class="search-date payment_time_from date-date date-range">
                        <?= $form->field($model, 'payment_time_from')->widget(DatePicker::class, [
                            'template' => '{input}<span class="input-group-addon1 clear-value"><span class="fa fa-times"></span></span>{addon}',
                            'clientOptions' => [
                                'format' => 'dd-mm-yyyy',
                                'autoclose' => true,
                    ...
                    <div class="date-range <?= $cls_search_date_pay ?> search-date-text text-to">-</div>
                    <div class="search-date payment_time_to date-range <?= $cls_search_date_pay ?>">
                        <?= $form->field($model, 'payment_time_to')->widget(DatePicker::class, [
                            'template' => '{input}<span class="input-group-addon1 clear-value"><span class="fa fa-times"></span></span>{addon}',
                            'clientOptions' => [
                                'format' => 'dd-mm-yyyy',
                                'autoclose' => true,
            ...
            <div class="row" style="padding-top:3px;padding-bottom:8px">
                ...
                <div class="form-search-col co_so col-md-4 col-12">
                    <div class="form-group row ml-0 mr-0">
                        <div class="f-content">
                            <div class="search-code">
                                <?= $form->field($model, 'tu_van_vien', ['template' => '<div class="input-group">{input}<span class="input-group-addon clear-option"><span class="fa fa-times"></span></span></div>'])
                                    ->dropDownList(\common\models\User::getNhanVienIsActiveArray(), ['class' => 'ui dropdown form-control', 'prompt' => 'Chọn tư vấn viên...'])->label(false) ?>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="form-search-col co_so col-md-4 col-12">
                    <div class="form-group row ml-0 mr-0">
                        <div class="f-content">
                            <div class="search-code">
                                <?php
                                echo $form->field($model, 'direct_sale_id', ['template' => '<div class="input-group">{input}<span class="input-group-addon clear-option"><span class="fa fa-times"></span></span></div>'])
                                    ->dropDownList(ArrayHelper::map(common\models\User::getNhanVienTuDirectSale(), 'id', 'fullname'), ['class' => 'ui dropdown form-control', 'prompt' => 'Chọn direct sale...'])->label(false) ?>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="f-content">
                            <div class="search-code">
                                ...
                                echo $form->field($model, 'id_dich_vu', ['template' => '<div class="input-group">{input}<span class="input-group-addon clear-option"><span class="fa fa-times"></span></span></div>'])
                                    ->dropDownList(Dep365CustomerOnlineDichVu::getSanPhamDichVuArray(), ['class' => 'ui dropdown form-control', 'prompt' => 'Chọn Dịch vụ...'])
                                    ->label(false);
                                ?>
                            </div>
                        </div>
