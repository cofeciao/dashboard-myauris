...
/* @var $searchModel backend\modules\clinic\models\search\PhongKhamDichVuSearch */
...
<section id="dom">
    ...
    [
                        ...
                        <?php Pjax::begin(['id' => 'clinic-dich-vu', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
...
clinic.init({
    pjaxId: '#clinic-dich-vu',
    urlChangePageSize: '$urlChangePageSize'
})
$('body').on('beforeSubmit', 'form#clinicDichVuAjax', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#clinicDichVuAjax').serialize();

    $('#clinicDichVuAjax').myLoading({opacity: true});
   
    $.ajax({
        url: $('#clinicDichVuAjax').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#clinicDichVuAjax').myUnloading();
        if (res.status == 200) {
