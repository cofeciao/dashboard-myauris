...
/* @var $searchModel backend\modules\customer\models\search\Dep365CustomerOnlineStatusSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Trạng thái khách hàng');
...
<section id="dom">
    ...
    [
                        ...
                        <?php Pjax::begin(['id' => 'customer-online-status', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
...
//                                        var_dump($userCreatedBy->fullname);die;
...
customerOnline.init({
    pjaxId: '#customer-online-status',
    urlChangePageSize: '$urlChangePageSize'
});

$('body').on('beforeSubmit', 'form#form-customer-status', function(e){
    e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-customer-status').serialize();
    
    $('#form-customer-status').myLoading({opacity: true});
       
    $.ajax({
        url: $('#form-customer-status').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-customer-status').myUnloading();
        if (res.status == 200) {
