...
/* @var $searchModel backend\modules\customer\models\Dep365CustomerOnlineFailStatusSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Lý do Fail');
...
<section id="dom">
    ...
    [
                        ...
                        <?php Pjax::begin(['id' => 'customer-fail-status', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
...
//                                'filterModel' => $searchModel,
                                ...
                                'columns' => [
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            $user = new backend\modules\customer\models\Dep365CustomerOnlineFailStatus();
...
customerOnline.init({
    pjaxId: '#customer-fail-status',
    urlChangePageSize: '$urlChangePageSize'
});

$('body').on('beforeSubmit', 'form#form-fail-status', function(e){
    e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-fail-status').serialize();
    
    $('#form-fail-status').myLoading({opacity: true});
       
    $.ajax({
        url: $('#form-fail-status').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-fail-status').myUnloading();
        if (res.status == 200) {
