...
/* @var $searchModel backend\modules\customer\models\search\Dep365CustomerOnlineDathenStatusSearch */
...
<section id="dom">
    ...
    [
                        ...
                        <?php Pjax::begin(['id' => 'customerOnlineDathenStatus', 'timeout' => false, 'enablePushState' => false, 'clientOptions' => ['method' => 'GET']]); ?>
...
customerOnline.init({
    pjaxId: '#customerOnlineDathenStatus',
    urlChangePageSize: '$urlChangePageSize'
});

$('body').on('beforeSubmit', 'form#form-dathen-status', function(e){
    e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-dathen-status').serialize();
    
    $('#form-dathen-status').myLoading({opacity: true});
       
    $.ajax({
        url: $('#form-dathen-status').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-dathen-status').myUnloading();
        if (res.status == 200) {
