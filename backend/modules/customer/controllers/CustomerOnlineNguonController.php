 ...
 */
class CustomerOnlineNguonController extends MyController
{
    ...
    {
        $searchModel = new Dep365CustomerOnlineNguonSearch();
    ...
    public function actionCreate()
    {
        if (Yii::$app->request->isAjax) {
            $model = new Dep365CustomerOnlineNguon();

            if ($model->load(Yii::$app->request->post()) && $model->validate()) {
                Yii::$app->response->format = Response::FORMAT_JSON;

                try {
                    $model->agency_id = json_encode($model->agency_id);
                    if ($model->save()) {
                        return [
                            'status' => 200,
    ...
    {
        if (($model = Dep365CustomerOnlineNguon::findOne($id)) !== null) {
            return $model;
        }
