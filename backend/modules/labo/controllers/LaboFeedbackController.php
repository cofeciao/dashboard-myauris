...
use backend\modules\labo\models\search\SearchLaboFeedback;
...
/**
 * LaboFeedbackController implements the CRUD actions for LaboFeedback model.
 */
class LaboFeedbackController extends MyController
{
    public function actionIndex()
    {
        $searchModel = new SearchLaboFeedback();
    ...
    /**
     * Updates an existing LaboFeedback model.
    ...
    public function actionDeleteOne()
    {
        if (Yii::$app->request->isGet) {
            ...
            try {
                if ($this->findModel($id)->delete()) {
                    Yii::$app->session->setFlash('alert', [
                        'body' => 'Xóa bình luận thành công',
                        'class' => 'bg-success',
                    ]);
                    return $this->redirect(Yii::$app->request->referrer);
                } else {
                    Yii::$app->session->setFlash('alert', [
                        'body' => 'Xóa bình luận thất bại',
                        'class' => 'bg-danger',
                    ]);
                    return $this->redirect(Yii::$app->request->referrer);
                }
    ...
    {
        if (($model = LaboFeedback::findOne($id)) !== null) {
            return $model;
        }
