...
/* @var $searchModel backend\modules\testab\models\search\AbAddKythuatSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Phân loại kỹ thuật');
...
<section id="dom">
    ...
    [
                        ...
                        <?php Pjax::begin(['id' => 'testab-ky-thuat', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
...
//                                'filterModel' => $searchModel,
                                ...
                                'columns' => [
                                    ...
                                    [
                                        ...
                                        'value' => function ($model) {
                                            $user = new backend\modules\testab\models\AbAddKythuat();
...
$script = <<< JS
var testAB = new myGridView({
    pjaxId: '#testab-ky-thuat',
    urlChangePageSize: '$urlChangePageSize',
});

$('body').on('beforeSubmit', 'form#form-testab-kythuat', function(e) {
   e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-testab-kythuat').serialize();

    $('#form-testab-kythuat').myLoading({opacity: true});
   
    $.ajax({
        url: $('#form-testab-kythuat').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-testab-kythuat').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: testAB.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: testAB.options.pjaxId});
                    }
