...
/* @var $searchModel backend\modules\booking\models\search\TimeWorkSearch */
/* @var $dataProvider yii\data\ActiveDataProvider */

$this->title = Yii::t('backend', 'Time Work');
...
<section id="dom">
    ...
    [
                        ...
                        <?php Pjax::begin(['id' => 'booking-time-work', 'timeout' => false, 'enablePushState' => true, 'clientOptions' => ['method' => 'GET']]); ?>
...
$script = <<< JS
var booking = new myGridView();
booking.init({
    pjaxId: '#booking-time-work',
    urlChangePageSize: '$urlChangePageSize'
});

$('body').on('beforeSubmit', 'form#form-time-work', function(e){
    e.preventDefault();
   var currentUrl = $(location).attr('href');
   var formData = $('#form-time-work').serialize();
    
    $('#form-time-work').myLoading({
        opacity: true
    });
       
    $.ajax({
        url: $('#form-time-work').attr('action'),
        type: 'POST',
        data: formData,
        dataType: 'json',
    })
    .done(function(res) {
        $('#form-time-work').myUnloading();
        if (res.status == 200) {
            $.when($.pjax.reload({url: currentUrl, method: 'POST', container: booking.options.pjaxId})).done(function(){
              ...
              $.ajax({
                ...
                success: function(data){
                    if(data.status == 'success') {
                        ...
                        $.pjax.reload({url: currentUrl, method: 'POST', container: booking.options.pjaxId});
                    }
