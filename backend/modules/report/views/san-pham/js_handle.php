...
$('body').on('change', '.filter-data-from-online-report, #direct-sale, #id-location, #id-page, #id-dichvu, #fan-page', function() {
    callData();
});
...
function callData() {
    $('#overview-graph-lineChart, #table-report-sanpham, #overview-miniPie').css('border', '1px solid #CDCDCD');
    $('#overview-graph-lineChart, #table-report-sanpham, #overview-miniPie').myLoading({msg:'Đang tải dữ liệu...'});

   id_location = $('#id-location').val();
   direct_sale = $('#direct-sale').val();
   id_page = $("#id-page").val();
   id_dichvu = $("#id-dichvu").val();
   ...
   $.ajax({
       ...
       success: function (data) {
           $('#overview-graph-lineChart, #table-report-sanpham, #overview-miniPie').myUnloading();
           $('#overview-graph-lineChart, #table-report-sanpham, #overview-miniPie').css('border', 'none');
           drawChart(data.data.date,data.data.result, data.data.lable);
           buildTable(data.data.tableData);
...
function buildTable(tableData){
    ...
    for (let i = 0; i < tableData.length; i++) {
        ...
        html += '<tr><td>'+tableData[i].name+'</td>';
